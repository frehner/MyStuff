...
/**
 * A singleton object that loads/saves business objects to/from a relational
 * database.
 * 
 * To use this class, adhere to the following:
 * 
 * 1. The database table name is the lowercase version of the Business Object
 * name. For example, the Employee.java BO should have a DB tabled named
 * "employee".
 * 
 * 2. The business object extends the BusinessObject.java class.
 * 
 * 3. The database table column names are the lowercase versions of the Business
 * Object names. For example, the "firstName" BO field matches the "firstname"
 * DB column name.
 * 
 * 4. The BO fields that match DB columns are annotated with
 * 
 * @BusinessObjectField keywords just before them. This DAO looks for these
 *                      annotations to know which fields it should set and read
 *                      values for.
 * 
 *                      5. Getters and setters exist for all annotated fields in
 *                      the Business Object.
 * 
 *                      6. A businessobject table exists in the database with
 *                      two columns: id (CHAR 40 PRIMARY KEY) and botype
 *                      (VARCHAR 250). All other tables in the database should
 *                      "extend" from this table, meaning that their id's are
 *                      foreign keys to this table.
 * 
 *                      7. The currently-supported field types are String,
 *                      int/Integer, long/Long, float/Float, double/double,
 *                      java.util.Date, and boolean/Boolean. Note that Java DB
 *                      (Derby) doesn't support boolean fields.
 * 
 *                      See the Tester.java class for examples on the use of
 *                      this class.
 * 
...
public class BusinessObjectDAO {
	...
	/**
	 * Interrogates an object and retrieves all fields and types that are
	 * annotated with "BusinessObjectField"
	 */
	...
	/**
	 * Creates a new BO in the database. The boType should be the simple class
	 * name of the BO to create. For example, "Employee" to build an employee
	 * BO. The id of the new business object is automatically generated using
	 * the GUID class.
	 */
	 ...
	 * BO.
	 * 
	 * This method allows the user to specify the id (GUID) of the new business
	 * object. This is useful when testing.
	 */
	...
	/**
	 * Reads an existing bo from the database. The method first checks the
	 * cache, then goes to the database if the object is not already in memory.
	 * The method discovers the business object type by querying the database,
	 * and the correct type is returned. Callers do not have to downcast the
	 * returned object.
	 */
	...
	synchronized <T extends BusinessObject> T read(String id, Connection conn) throws Exception {
		...
		// get it's type from the businessobject table so we know what table to
		// query
		...
		// initialize the data of the object by stepping through its
		// superclasses all the way to BusinessObject
	...
	/**
	 * Saves an existing business object in the database. The method
	 * automatically determines whether an INSERT or UPDATE call should be
	 * invoked. The object must be set "dirty" for the save to run.
	 */
	public <T extends BusinessObject> void save(T bo) throws DataException {
		...
		Connection conn = ConnectionPool.getInstance().get();
		try {
			// get the superclasses of this object, putting them in reverse
			// order (since they need to save that way for referential
			// integrity).
			...
			// step through the class hierarchy under BusinessObject and let
			// each save to the database
	...
	private <T extends BusinessObject> void update(T bo, Class boClass, Connection conn) throws Exception {
		...
		String[] fieldNames = fields.keySet().toArray(new String[fields.size()]); 
		
		// so the order can't change between the two for loops			
	...
	private <T extends BusinessObject> void insert(T bo, Class boClass, Connection conn) throws Exception {
		...
		Map<String, Class> fields = getBusinessObjectFields(boClass);
		String[] fieldNames = fields.keySet().toArray(new String[fields.size()]); // so
																					// the
																					// order
																					// can't
																					// change
	...
	/**
	 * Deletes a business object from the database. Normally, BOs should not be
	 * deleted (only "inactivated"). This DAO supports delete because there are
	 * times when deleting should occur, such as when an association class needs
	 * a record removed.
	 */
	public <T extends BusinessObject> void delete(T bo) throws DataException {
		Connection conn = ConnectionPool.getInstance().get();
		try {
			// go through each level of the class hierarchy and delete on each
			// table
			Class boClass = bo.getClass();
			while (true) {
	...
	/**
	 * Retrieves the BOs from the database that meet the given criteria. Use
	 * this method when you expect multiple BOs to match the criteria. If no BOs
	 * match the criteria, an zero-size list is returned.
	 * 
	 * Use one or more SearchCriteria objects to specify the search.
	 * 
	 * Search for employees with name=Bart: searchForList("Employee", new
	 * SearchCriteria("name", "Bart"));
	 * 
	 * Search for employees with username=Bart and salary > 500:
	 * searchForList("Employee", new SearchCriteria("username", "bart"), new
	 * SearchCriteria("salary", 500, SearchCriteria.GREATER_THAN);
	 * 
	 * Search for employees with names starting with "B".
	 * searchForList("Employee", new SearchCriteria("name", "B%",
	 * SearchCriteria.LIKE));
	 * 
	 */
	 ...
	 * Use this method when you expect only one BO to match the criteria. If no
	 * BOs match the criteria, null is returned.
	 * 
	 * Use one or more SearchCriteria objects to specify the search.
	 * 
	 * Search for employees with name=Bart: searchForList("Employee", new
	...
	public <T extends BusinessObject> T searchForBO(String boType, SearchCriteria... criteria) throws DataException {
		// this is not the most efficient way to do this, but it is simple and
