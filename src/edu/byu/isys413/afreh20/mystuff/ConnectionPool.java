 ...
 * Version 2008-03-03
 * 
 * A connection pool for database connections. Note that javax.sql has a
 * standard connection pool interface, but this does not implement it. This is a
 * bare bones connection pool and is not robust or efficient enough to be used
 * in the real world. It is just for example purposes, and it assumes that we
 * need simplicity over function.
 * 
...
public class ConnectionPool {
	...
	// "jdbc:derby://localhost:1527/tester";
	// private static String DRIVER_NAME = "org.apache.derby.jdbc.ClientDriver";

//	private static String DATABASE_URL = "jdbc:derby:C:/Users/Tony & Lisa/MyStuffDb;create=true";
//	private static String DRIVER_NAME = "org.apache.derby.jdbc.EmbeddedDriver";
	
	private static String DATABASE_URL = "jdbc:mysql://localhost:3306/mystuffdb";
	  private static String DATABASE_USERNAME = "root";
	  private static String DATABASE_PASSWORD = "passworddb";
	...
	public synchronized Connection get() throws DataException {
		...
		if (usedConnections.size() >= MAX_CONNECTIONS) {
			throw new DataException(
					"The database connection pool is out of connections -- a maximum number of "
							+ MAX_CONNECTIONS);
		}// if
		...
		try {
			...
			// Logger.global.info("Gave out a connection from the pool.  Free size is now: "
			// + freeConnections.size() + "/" + (freeConnections.size() +
			// usedConnections.size()));
			return conn;
		} catch (Exception e) {
			throw new DataException(
					"An error occurred while retrieving a database connection from the pool",
					e);
		}
	}// get
	...
	public synchronized void release(Connection conn) throws DataException {
		try {
			// be sure that this connection was committed (so it is at a fresh,
			// new transaction)
			conn.commit();
			...
			// Logger.global.info("Released a connection back to the pool.  Free size is now: "
			// + freeConnections.size() + "/" + (freeConnections.size() +
			// usedConnections.size()));
		} catch (Exception e) {
			throw new DataException(
					"An error occurred while releasing a database connection back to the pool",
					e);
		}
	}// release
